...
import type { SphrexAttachment } from '@generated/types';
 ...
 */
const uploadToIPFS = async (data: any): Promise<SphrexAttachment[]> => {
  try {
 ...
 */
export const uploadFileToIPFS = async (file: File): Promise<SphrexAttachment | null> => {
  try {
    const client = await getS3Client();
    const params = {
      Bucket: S3_BUCKET.SPHREX_MEDIA,
