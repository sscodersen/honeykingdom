import type { NewSphrexAttachment } from '@generated/types';
...
const useUploadAttachments = () => {
  ...
  const handleUploadAttachments = useCallback(
    async (attachments: any): Promise<NewSphrexAttachment[]> => {
      setIsUploading(true);
      const files = Array.from(attachments);
      const attachmentIds: string[] = [];
      const previewAttachments: NewSphrexAttachment[] = files.map((file: any) => {
      ...
      let attachmentsIPFS: NewSphrexAttachment[] = [];
      try {
        const attachmentsUploaded = await uploadToIPFS(attachments);
        if (attachmentsUploaded) {
          attachmentsIPFS = previewAttachments.map((attachment: NewSphrexAttachment, index: number) => ({
            ...attachment,
